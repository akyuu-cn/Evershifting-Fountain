...
打开 Github 用户设置 `Developer settings` - `Personal access tokens`，选择 `Generate new token`，`Expiration` 选择 `no expiration`（永不过期），下方勾选 repo 所有权限，然后点击 `Generate token`。
...
点击仓库 `Settings` - `Secrets` - `Actions`，右上角选择 `New repository secret`， `Name` 填写 `GH_TOKEN`，然后把刚才复制下来的 token 粘贴进去。
...
然后进行一次 Commit，生成好的文件就会自动上传到服务器网站根目录了。
